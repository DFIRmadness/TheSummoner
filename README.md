...
Wraith carries a per-scenario campaign tag string embedded at deploy time. The encoding is scaled to difficulty:

- **Easy** — plaintext; `strings wraith.exe` finds it immediately
- **Medium** — XOR key `0x42`; FLOSS recovers it automatically; `strings` misses it
- **Hard** — `base64(XOR 0x42)`; requires FLOSS emulation plus a manual decode step
...
Analysts learn to compute and verify SHA256 hashes, identify suspicious file drop locations by difficulty level, correlate process metadata (PID, name, path, parent) with network telemetry, and enumerate persistence mechanisms across the registry, cron, systemd, and startup folders. This mirrors the workflow used in real incident response.
...
| **Persistence detection** | Do your integrity monitors catch the cron job, registry key, or systemd unit? |
...
Outputs:
```
summoner_linux_amd64
summoner_windows_amd64.exe
```

The build compiles Wraith first for both platforms, then embeds the binaries into Summoner via `//go:embed`.
...
| **Campaign tag encoding** | Plaintext | XOR 0x42 | base64(XOR 0x42) |
...
| `--mechanism` | auto | Persistence mechanism ID |
...
| Campaign tag | `strings`, FLOSS, Wireshark User-Agent header |
...
| ID | Platform | Location | Needs elevation? |
|---|---|---|---|
| `user_crontab` | Linux | `crontab -l` | No |
| `bashrc` | Linux | `~/.bashrc` | No |
| `systemd_user` | Linux | `~/.config/systemd/user/` | No |
| `systemd_system` | Linux | `/etc/systemd/system/` | Yes |
| `etc_crond` | Linux | `/etc/cron.d/` | Yes |
| `hkcu_run` | Windows | `HKCU\...\CurrentVersion\Run` | No |
| `startup_folder` | Windows | User Startup folder | No |
| `hklm_run` | Windows | `HKLM\...\CurrentVersion\Run` | Yes |
| `scheduled_task` | Windows | Task Scheduler | Yes |
